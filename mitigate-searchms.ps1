<#
This will mitigate (CVE-???) search-ms zeroday according to the recommandation of Micro$oft's follina zero day
...
param(
    ...
    [string]$backuppath="C:\temp\searchms-mitigate", # The path, where the reg-backup should be stored
    [string]$backupfilename="searchms_bak.reg", # The filename to where the reg-backup should be written
    [string]$logfile="C:\temp\searchms-mitigate.log" # logfile
)
...
if (!(Test-Path -path "Registry::HKEY_CLASSES_ROOT\search-ms")){
...
Invoke-Command  {reg export 'HKEY_CLASSES_ROOT\search-ms' $bakfil.FullName /y} # Export the registry key. overwrite existing backupfile (if any)
...
if ($bakfil.Exists){
    ...
    try {
        ...
        Invoke-Command  {reg delete 'HKEY_CLASSES_ROOT\search-ms' /f} # Delete the key
    }
    catch {
